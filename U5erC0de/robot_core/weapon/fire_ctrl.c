...
void est_position_filter_init(void) {
  LPF_init(&vx_filter, 1.f / fs_tim_freq, base_vel_fc);
  LPF_init(&vy_filter, 1.f / fs_tim_freq, base_vel_fc);
  LPF_init(&vz_filter, 1.f / fs_tim_freq, base_vel_fc);
...
float shootable_yaw_angle = 0.75;
float shootable_pitch_angle = 0.67;
...
void get_vision_suggest_fire(gimbal_state_t *gimbal_expt_state,
  ...
  if (shoot_delta_yaw_angle < deg2rad(shootable_yaw_angle) &&
      shoot_delta_pitch_angle - 0.11 < deg2rad(shootable_pitch_angle) &&
      // 云台跟随情况
...
uint8_t get_vision_ctrl(float *pitch_ang, float *yaw_ang, float dt) {
  if (vision_ctrl_data.target_found) {
    ...
    // 载入数据
    bullet_speed_error = referee_info.bullet_speed - cur_v0;
     yaw = vision_ctrl_data.yaw;
     r1 = vision_ctrl_data.r1;
     r2 = vision_ctrl_data.r2;
     xc = vision_ctrl_data.x;
     yc = vision_ctrl_data.y;
     zc = vision_ctrl_data.z;
     vx = vision_ctrl_data.vx;
     vy = vision_ctrl_data.vy;
     vz = vision_ctrl_data.vz;
     vyaw = vision_ctrl_data.v_yaw;
     dz = vision_ctrl_data.dz;
     armor_num = vision_ctrl_data.armor_num;
    // uint8_t id_num = vision_ctrl_data.id_num;
    extern float predict_time;
    extern float est_x, est_y, est_z, est_yaw, distance_xy;
    extern float best_attack_x, best_attack_y, best_attack_z, best_attack_yaw;
