...
/**
 *	@brief	选择目标
 *	@param	v0:氮素,由裁判系统+卡尔曼滤波而得
 *					target->x0:目标在xy平面距离
 *					target->z0:目标高度
 *					solution:解算返回值
 */
void projectile_solve(float v0, target_spec_t *target,
                      ballistic_sol_t *solution) {
  float x0x0 = target->x0 * target->x0; // 此处的x0应该是xy平面的距离
  float distance2 = x0x0 + target->z0 * target->z0;
  float distance = BALLISTIC_SQRT(distance2); // 计算目标的空间距离
  ...
  float alpha = BALLISTIC_ASIN(a / distance); // 近似计算
  float phi = M_PI / 2;
  if (target->x0 != 0.0f)
    phi = BALLISTIC_ATAN(target->z0 / target->x0);
...
void projectile_solve_optim(float v0, target_spec_t *target,
                            ballistic_sol_t *solution) {
  ...
  if (target->x0 != 0.0f)
    cordic_atan_sqrtf(target->z0, target->x0, &phi, &distance);
...
void ballistic_solve(float v0, target_spec_t *target,
                     ballistic_sol_t *solution) {
  float v0v0 = v0 * v0;
  float x0x0 = target->x0 * target->x0;
  ...
  for (int i = 0; i < MAX_ITERATION_NUM; i++) {
    ...
    if (poly_ln > 0 && t0 > t0_min) {
      ...
      if (solution->ang_solution1 <= 0)
        poly_sqrt = -poly_sqrt;
      // poly_sqrt = sqrtf(v0v0*t0*t0 - x0x0);
      ft0 = GRAVITY_G / K_AIR * (BALLISTIC_LN(poly_ln) / K_AIR + t0) +
            poly_sqrt - target->z0;
...
void ballistic_solve_optim(float v0, target_spec_t *target,
                           ballistic_sol_t *solution) {
  float v0v0 = v0 * v0;
  float x0x0 = target->x0 * target->x0;
  float t0_min = target->x0 / v0;
  ...
  for (int i = 0; i < MAX_ITERATION_NUM; i++) {
    if (t0 < 1 / K_AIR && t0 > t0_min) {
      ...
      if (solution->ang_solution1 <= 0)
        poly_sqrt = -poly_sqrt;
      // poly_sqrt = sqrtf(v0v0*t0*t0 - x0x0);
      // ft0 = GRAVITY_G/K_AIR*(BALLISTIC_LN(poly_ln)/K_AIR + t0) + poly_sqrt -
      // z0;
      ...
      ln_val += (1 + (K_AIR / 2 + K_AIR * K_AIR / 3 * ln_series) * ln_series) *
                ln_series;
    } else {
      solution->solution_num = 0;
      break;
