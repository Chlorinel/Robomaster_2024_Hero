...
void init_imu_dcm(dcm_init_t *init_para)
{
	...
	{
		P00 = init_para->init_dcm_variance;
		P01 = 0;
		P02 = 0;
		P03 = 0;
		P04 = 0;
		P05 = 0;
		P11 = init_para->init_dcm_variance;
		P12 = 0;
		P13 = 0;
		P14 = 0;
		P15 = 0;
		P22 = init_para->init_dcm_variance;
		P23 = 0;
		P24 = 0;
		P25 = 0;
		P33 = init_para->init_bias_variance;
		P34 = 0;
		P35 = 0;
		P44 = init_para->init_bias_variance;
		P45 = 0;
...
void imu_dcm_get_eular(eular_t *_eular)
{
	...
	_eular->yaw = atan2f(sr0, fr0);
	// compute new pitch and roll angles from a posteriori states
	_eular->pitch = asinf(-x0);
	_eular->roll = atan2f(x1, x2);
}
...
void imu_dcm_update(imu_data_fp_t *_imu, imu_gyro_cal_t *gb, float dt)
{
	...
	if (((GRAVITY_G0 * recipNorm) < (1 + STEADY_ACCEL_RANGE)) && ((GRAVITY_G0 * recipNorm) > (1 - STEADY_ACCEL_RANGE)) && fabsf(_imu->gx) < STEADY_GYRO_RANGE && fabsf(_imu->gy) < STEADY_GYRO_RANGE && fabsf(_imu->gz) < STEADY_GYRO_RANGE)
	{
		...
		else
		{
			...
			gb->gx_bias = CLAMP_MAX(gb->gx_bias, GYRO_BIAS_MAX_RAW);

			gb->gy_bias += BIAS_ALPHA * (_imu->gy);
			gb->gy_bias = CLAMP_MAX(gb->gy_bias, GYRO_BIAS_MAX_RAW);

			gb->gz_bias += BIAS_ALPHA * (_imu->gz);
			gb->gz_bias = CLAMP_MAX(gb->gz_bias, GYRO_BIAS_MAX_RAW);
			//		_imu->gx = 0.0f;
			//		_imu->gy = 0.0f;
			//		_imu->gz = 0.0f;
		}
	...
	x3 = CLAMP_MAX(x3, GYRO_BIAS_MAX);
	//	//x3 = CLAMP_MAX((x_3 + K30*y0 + K31*y1 + K32*y2), GYRO_BIAS_MAX);
	x4 = x_4 + K40 * y0 + K41 * y1 + K42 * y2;
	x4 = CLAMP_MAX(x4, GYRO_BIAS_MAX);
	//	//x4 = CLAMP_MAX((x_4 + K40*y0 + K41*y1 + K42*y2), GYRO_BIAS_MAX);
	x5 = x_5 + K50 * y0 + K51 * y1 + K52 * y2;
	x5 = CLAMP_MAX(x5, GYRO_BIAS_MAX);
	//	//x5 = CLAMP_MAX((x_5 + K50*y0 + K51*y1 + K52*y2), GYRO_BIAS_MAX);
#else
	x3 = 0.0f;
	x4 = 0.0f;
	x5 = 0.0f;
	...
	if (P00 < VARIANCE_MIN_LIMIT)
		P00 = VARIANCE_MIN_LIMIT;
	if (P11 < VARIANCE_MIN_LIMIT)
		P11 = VARIANCE_MIN_LIMIT;
	if (P22 < VARIANCE_MIN_LIMIT)
		P22 = VARIANCE_MIN_LIMIT;
	if (P33 < VARIANCE_MIN_LIMIT)
		P33 = VARIANCE_MIN_LIMIT;
	if (P44 < VARIANCE_MIN_LIMIT)
		P44 = VARIANCE_MIN_LIMIT;
	if (P55 < VARIANCE_MIN_LIMIT)
		P55 = VARIANCE_MIN_LIMIT;
